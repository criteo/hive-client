...
class HiveThriftDriver(addr: String, port: Int, logger: HiveClientLogger)(implicit ec: ExecutionContext) {
  ...
  def logOperation(operationHandle: TOperationHandle)(outFn: Iterable[Seq[Any]] => Unit): Future[Unit] = Future {
    ...
    case TOperationState.FINISHED_STATE => Future.successful()
    case TOperationState.ERROR_STATE => Future.failed(new HiveException("Error, please check HiveServer2 logs"))
    case TOperationState.CANCELED_STATE => Future.failed(new HiveException("Error, job is cancelled."))
    case _ => Utils.timeout(Duration.ofMillis(500)).flatMap(u => logOperation(operationHandle)(outFn))
  }
